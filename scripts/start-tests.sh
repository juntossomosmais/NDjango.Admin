...
# When sourced (e.g. by dotnet-test-sonar.sh), use return so the caller can continue (e.g. run sonar end).
# When run as a script, use exit so the process exits with the test result.
